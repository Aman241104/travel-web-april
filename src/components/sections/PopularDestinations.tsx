    ...
    <section id="packages" ref={containerRef} className="relative py-16 lg:py-28 bg-[#020504] overflow-hidden scroll-mt-24">
      ...
      <div className="container-custom relative z-10">
        ...
        <div className="dest-header flex flex-col lg:flex-row lg:items-end justify-between mb-10 lg:mb-20 gap-8 lg:gap-12">
          <div className="max-w-4xl space-y-6 lg:space-y-8">
            ...
            <h2 className="text-[34px] md:text-[52px] lg:text-[68px] xl:text-[84px] font-sans font-black text-white leading-[0.95] lg:leading-[1] tracking-tightest uppercase">
              ...
              <span className="block dest-headline-line text-transparent bg-clip-text bg-gradient-to-r from-primary via-white to-primary/40 italic font-serif font-light lowercase normal-case py-1">exclusive</span>
              <span className="block dest-headline-line">Horizon.</span>
            </h2>
            
            <p className="text-gray-500 text-[13px] lg:text-xl leading-relaxed max-w-2xl font-medium tracking-tight opacity-90">
          ...
          <div className="flex flex-col gap-6 lg:gap-8">
            <div className="flex gap-4 lg:gap-6">
              <button 
                onClick={() => scroll("left")}
                className="w-14 h-14 lg:w-20 lg:h-20 rounded-full border border-white/10 flex items-center justify-center bg-white/[0.02] text-white hover:bg-primary hover:border-primary transition-all duration-700 shadow-2xl group active:scale-90"
              >
        ...
        <div 
          ...
          className="flex gap-5 lg:gap-16 overflow-x-auto no-scrollbar pb-12 lg:pb-32 snap-x snap-mandatory px-0"
        >
          {destinations.map((dest, i) => (
            <div
              key={i}
              onClick={() => handleDestinationClick(dest)}
              className="dest-card min-w-[320px] md:min-w-[480px] lg:min-w-[750px] group snap-start relative h-[480px] lg:h-[700px] rounded-[32px] lg:rounded-[64px] overflow-hidden cursor-pointer shadow-[0_50px_100px_rgba(0,0,0,0.6)] border border-white/5 bg-gray-950 transition-all duration-1000 active:scale-[0.98]"
            >
              <Image 
                src={dest.image} 
                alt={dest.name} 
                fill 
                className="object-cover transition-transform duration-[6s] group-hover:scale-110 ease-out brightness-[0.7] group-hover:brightness-90" 
              />
              ...
              <div className="absolute top-6 lg:top-10 right-6 lg:right-10 z-30 overflow-hidden rounded-full">
                <div className="bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-full px-4 py-2.5 lg:px-10 lg:py-6 shadow-2xl flex items-center gap-3 lg:gap-4 group-hover:bg-primary transition-all duration-1000">
                  <div className="flex flex-col items-end">
                    <span className="text-[7px] lg:text-[10px] font-black text-white/50 uppercase tracking-widest group-hover:text-white transition-colors">Starting At</span>
                    <span className="text-sm lg:text-3xl font-black text-white tracking-tightest">₹{dest.price}</span>
                  </div>
              ...
              <div className="absolute bottom-4 left-4 right-4 lg:bottom-12 lg:left-12 lg:right-12 z-30">
                <div className="relative p-5 lg:p-14 rounded-[28px] lg:rounded-[56px] bg-white/[0.04] backdrop-blur-3xl border border-white/10 overflow-hidden group/info transition-all duration-1000 group-hover:bg-white/[0.08] group-hover:border-white/20">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center justify-between mb-4 lg:mb-12">
                      <div className="flex items-center gap-2.5 lg:gap-4 text-primary font-black text-[8px] lg:text-[12px] uppercase tracking-[0.4em] lg:tracking-[0.6em] transition-all duration-1000 group-hover:translate-x-4">
                        <Globe className="w-3.5 h-3.5 lg:w-6 lg:h-6" />
                        {dest.location}
                      </div>
                      <div className="flex items-center gap-1 px-2.5 py-0.5 lg:px-3 lg:py-1 bg-white/5 rounded-full border border-white/10">
                        <Star className="w-2.5 lg:w-5 h-2.5 lg:h-5 text-accent-gold fill-accent-gold" />
                        <span className="text-[9px] lg:text-[12px] font-black text-white tracking-widest">{dest.rating}</span>
                      </div>
                    </div>
                    
                    <h3 className="text-[26px] lg:text-[56px] font-sans font-black text-white mb-5 lg:mb-14 leading-[0.9] tracking-tightest uppercase transition-all duration-1000 group-hover:text-primary group-hover:scale-105 origin-left">
                      {dest.name}
                    </h3>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="w-11 h-11 lg:w-24 lg:h-24 rounded-[16px] lg:rounded-[36px] bg-primary text-white flex items-center justify-center shadow-3xl transition-all duration-1000 hover:bg-white hover:text-primary group-hover:scale-110 group-hover:rotate-6">
                        <ArrowRight className="w-5 h-5 lg:w-12 lg:h-12" />
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="mt-16 lg:mt-32 flex flex-col lg:flex-row items-center justify-between gap-10 bg-white/[0.03] backdrop-blur-2xl rounded-[32px] lg:rounded-[64px] p-8 lg:p-16 border border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.4)] group/footer overflow-hidden relative">
          ...
          <div className="max-w-2xl space-y-5 relative z-10 text-center lg:text-left">
            <h4 className="text-[28px] lg:text-[48px] font-sans font-black text-white leading-[1] tracking-tightest uppercase">
              Seek Your <br className="hidden sm:block" />
              <span className="text-primary italic font-serif font-light lowercase normal-case">personal</span> Sanctuary.
            </h4>
            <p className="text-gray-500 text-sm lg:text-xl font-medium leading-relaxed max-w-xl opacity-90">
          ...
          <MagneticButton 
            ...
            className="w-full lg:w-auto px-10 py-5 lg:px-20 lg:py-8 bg-primary text-white font-black rounded-2xl lg:rounded-[32px] transition-all shadow-[0_30px_70px_rgba(56,142,60,0.4)] group flex items-center justify-center gap-4 lg:gap-6 uppercase tracking-[0.3em] lg:tracking-[0.4em] text-[10px] lg:text-xs relative overflow-hidden active:scale-95"
          >
            <div className="absolute inset-0 bg-white/20 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-in-out" />
            <span className="relative z-10">Commission A Journey</span>
            <ArrowRight className="w-5 h-5 lg:w-8 lg:h-8 relative z-10 group-hover:translate-x-3 transition-transform duration-700" />
          </MagneticButton>
        </div>
      </div>
    </section>
