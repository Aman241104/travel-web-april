...
export default function ServicesList() {
  ...
  return (
    <section id="services" className="py-32 md:py-56 bg-[#0B1310] relative transition-colors duration-1000">
      <div className="container mx-auto px-6">
        <div className="max-w-4xl mb-32 text-[#F2EFE9]">
          ...
          <h2 className="font-serif text-6xl md:text-[100px] leading-tight tracking-tightest">
            ...
            Global <span className="text-[#F2EFE9]/30">Reach.</span>
          </h2>
        </div>

        <div ref={containerRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-[#0B1310]/10 border border-[#F2EFE9]/5 rounded-[2px] overflow-hidden shadow-2xl">
          {services.map((service, index) => (
            <div 
              key={index} 
              className="service-item group p-12 bg-[#0B1310] hover:bg-bg-light transition-all duration-700 cursor-pointer relative"
            >
              <div className="flex items-start justify-between mb-20">
                <div className="w-10 h-10 flex items-center justify-center text-[#F2EFE9] group-hover:text-accent-blue transition-colors duration-500">
                  <service.icon className="w-6 h-6 stroke-[1.5px]" />
                </div>
                <span className="font-sans text-[10px] font-black text-[#F2EFE9]/10 tracking-widest">
                  {index + 1 < 10 ? `0${index + 1}` : index + 1}
                </span>
              </div>
              
              <h3 className="text-[#F2EFE9] font-sans text-[11px] font-black uppercase tracking-[0.3em] mb-6 group-hover:text-accent-blue transition-colors duration-500">
                {service.title}
              </h3>
              <p className="text-[#F2EFE9]/50 text-xs leading-relaxed font-medium group-hover:text-[#0B1310] transition-colors duration-500">
                {service.desc}
              </p>
