...
const services = [
  ...
  { 
    ...
    icon: Plane, 
    image: "https://images.unsplash.com/photo-1540910419892-4a39d20b2944?q=80&w=1200&auto=format&fit=crop",
        ...
        {selectedService && (
          ...
          >
            ...
            <motion.div 
              ...
              className="relative w-full max-w-6xl bg-[#0B1310] border border-[#F2EFE9]/10 rounded-[2rem] md:rounded-[4rem] overflow-hidden shadow-2xl flex flex-col md:flex-row max-h-[85vh] md:max-h-[80vh]"
            >
              {/* Image Side */}
              ...
              {/* Content Side */}
              <div className="relative w-full md:w-1/2 p-5 md:p-16 lg:p-24 overflow-y-auto">
                <button 
                  onClick={() => setSelectedService(null)}
                  className="absolute top-5 right-5 p-2 md:p-3 rounded-full bg-[#F2EFE9]/5 hover:bg-[#C1A67B] hover:text-[#0B1310] transition-all duration-500 z-10"
                >
                  <X className="w-5 h-5 md:w-6 md:h-6" />
                </button>

                <div className="mb-6 md:mb-12">
                  <span className="text-[#C1A67B] font-sans text-[9px] md:text-xs font-bold uppercase tracking-[0.5em] mb-3 md:mb-6 block">
                    Service Definition
                  </span>
                  <h3 className="font-serif text-2xl md:text-6xl text-[#F2EFE9] leading-[0.9] tracking-tighter mb-4 md:mb-8">
                    {selectedService.title}
                  </h3>
                  <p className="text-[#F2EFE9]/60 font-sans text-sm md:text-xl leading-relaxed">
                    {selectedService.details}
                  </p>
                </div>

                <div className="space-y-4 md:space-y-8">
                  <h4 className="text-[#F2EFE9] font-sans text-[9px] md:text-xs font-black uppercase tracking-[0.4em] border-b border-[#F2EFE9]/10 pb-3 md:pb-4">
                    Key Expertise
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-6">
                    {selectedService.features.map((feature, idx) => (
                      <div key={idx} className="flex items-center gap-2 md:gap-4 group/item">
                        <div className="w-1 md:w-1.5 h-1 md:h-1.5 rounded-full bg-[#C1A67B]" />
                        <span className="text-[#F2EFE9]/40 font-sans text-[8px] md:text-[11px] font-bold uppercase tracking-widest group-hover/item:text-[#F2EFE9] transition-colors">
                          {feature}
                        </span>
                      </div>
                    ))}
                ...
                <div className="mt-8 md:mt-16">
                  <button className="w-full group relative flex items-center justify-between px-6 py-3.5 md:px-10 md:py-6 bg-[#C1A67B] rounded-full text-[#0B1310] font-sans text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] overflow-hidden transition-all duration-700">
                    <span className="relative z-10">Inquire for Private Access</span>
                    <ArrowUpRight className="relative z-10 w-3.5 h-3.5 md:w-5 md:h-5 group-hover:rotate-45 transition-transform duration-500" />
