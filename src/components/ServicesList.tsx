...
export default function ServicesList() {
  return (
    <section id="services" className="py-20 lg:py-32 bg-gray-50 relative overflow-hidden border-t border-gray-100">
      ...
      <div className="container-custom relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 lg:mb-28 gap-8 lg:gap-12">
          <div className="max-w-[720px]">
            ...
            <h2 className="text-4xl lg:text-[84px] font-sans font-black text-gray-900 leading-[1.1] lg:leading-[1.05] tracking-tighter">
              <span className="text-primary italic font-serif font-light drop-shadow-sm">Services</span>
            </h2>
          </div>
          <p className="text-gray-600 lg:max-w-[440px] text-lg lg:text-xl leading-relaxed font-medium tracking-tight">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 mb-8 lg:mb-12">
          {featuredServices.map((service, i) => (
            <motion.div
              ...
              className={`p-8 lg:p-16 rounded-[32px] lg:rounded-[48px] relative overflow-hidden group border border-gray-100/50 ${
            ...
            >
              <div className="relative z-10 flex flex-col h-full">
                <div className={`w-16 h-16 lg:w-20 lg:h-20 rounded-2xl lg:rounded-[24px] flex items-center justify-center mb-8 lg:mb-12 shadow-lg transition-all duration-700 group-hover:rotate-[15deg] group-hover:scale-110 ${
                  service.color === 'bg-primary' ? "bg-white/20 backdrop-blur-md" : "bg-primary/10"
                }`}>
                  <service.icon className={`w-8 h-8 lg:w-10 lg:h-10 ${service.color === 'bg-primary' ? "text-white" : "text-primary"}`} />
                </div>
                <h3 className="text-2xl lg:text-[44px] font-sans font-bold mb-4 lg:mb-6 tracking-tight leading-tight">{service.title}</h3>
                <p className={`text-base lg:text-lg mb-10 lg:mb-14 leading-relaxed max-w-[480px] font-medium ${
                  service.color === 'bg-primary' ? "text-white/80" : "text-gray-500"
                }`}>
                  {service.desc}
                </p>
                <div className="mt-auto">
                  <button className={`flex items-center gap-4 font-black uppercase tracking-[0.25em] text-[10px] lg:text-[11px] group/btn transition-all duration-500 ${
                    service.color === 'bg-primary' ? "text-white hover:text-white" : "text-primary hover:text-primary-dark"
                  }`}>
                    {service.cta}
                    <div className={`w-8 lg:w-10 h-px transition-all duration-500 group-hover/btn:w-16 ${
                      service.color === 'bg-primary' ? "bg-white" : "bg-primary"
                    }`} />
                    <ArrowRight className="w-4 h-4 lg:w-5 lg:h-5 group-hover/btn:translate-x-2 transition-transform duration-500" />
                  </button>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {standardServices.map((service, i) => (
            <motion.div
              ...
              className="bg-white p-8 lg:p-10 rounded-3xl lg:rounded-[32px] border border-gray-100 hover:border-primary/20 hover:shadow-xl hover:shadow-primary/5 transition-all duration-700 group relative overflow-hidden"
            >
              <div className="flex flex-col gap-6 lg:gap-8 relative z-10">
                <div className="w-14 h-14 lg:w-16 lg:h-16 rounded-2xl lg:rounded-[20px] bg-gray-50 border border-gray-100 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all duration-700 shrink-0 shadow-sm transform group-hover:rotate-6">
                  <service.icon className="w-7 h-7 lg:w-8 lg:h-8" />
                </div>
                <div>
                  <h4 className="text-xl lg:text-2xl font-sans font-bold text-gray-900 mb-3 lg:mb-4 group-hover:text-primary transition-colors duration-500 leading-tight">
                    {service.title}
                  </h4>
        ...
        <motion.div 
          ...
          className="mt-20 lg:mt-32 flex flex-col items-center text-center"
        >
          <div className="bg-white rounded-full px-6 lg:px-8 py-2.5 lg:py-3 flex items-center gap-3 lg:gap-4 border border-gray-100 mb-8 lg:mb-12 shadow-sm">
            <div className="w-2 h-2 lg:w-2.5 lg:h-2.5 rounded-full bg-primary animate-pulse" />
            <p className="text-[10px] lg:text-[11px] font-black text-gray-900 uppercase tracking-[0.4em]">
              Elevate your travel standards
            </p>
          </div>
          
          <div className="flex flex-col sm:flex-row justify-center gap-4 lg:gap-6 w-full sm:w-auto">
            <Link 
              href="#contact" 
              className="w-full sm:w-auto px-10 py-5 lg:px-12 lg:py-6 bg-primary text-white font-bold rounded-full transition-all shadow-lg hover:shadow-primary/30 hover:bg-primary-dark hover:-translate-y-1 active:translate-y-0 text-sm uppercase tracking-widest relative overflow-hidden group"
            >
            ...
            <Link 
              href="#packages" 
              className="w-full sm:w-auto px-10 py-5 lg:px-12 lg:py-6 bg-white border-2 border-gray-100 text-gray-800 font-bold rounded-full transition-all hover:border-primary hover:text-primary shadow-sm hover:shadow-md hover:-translate-y-1 active:translate-y-0 text-sm uppercase tracking-widest"
            >
              Explore Collections
            </Link>
          </div>
